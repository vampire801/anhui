...
--]]
	...
	local msd_id = MsgIDDefine.LOBBY_GATEWAY_CLIENT_SERVER_REQ
  local msg = (MsgIDMap[msd_id])()
	msg.iServerId = 0
	LobbyGatewayConnector:GetInstance():SendMessage(msd_id, msg)
end
...
local function ConnectServer(self)
	LobbyGatewayConnector:GetInstance():Connect("192.168.1.199", 1031, Bind(self, OnConnect), Bind(self, OnClose))
end
