...
local function EnterLobby()
	-- 进入大厅
	SceneManager:GetInstance():SwitchScene(SceneConfig.LobbyScene)
end

local function EnterRoom()
	-- 进入大厅
	SceneManager:GetInstance():SwitchScene(SceneConfig.RoomScene)
end
-- GameMain公共接口，其它的一律为私有接口，只能在本模块访问

GameMain.EnterLobby=EnterLobby
GameMain.EnterRoom=EnterRoom
