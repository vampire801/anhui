...
local function Update(self)
	...
	local lobbyGatewayConnector = LobbyGatewayConnector:GetInstance()
	local status,err = pcall(lobbyGatewayConnector.Update, lobbyGatewayConnector)
	if not status then
		Logger.LogError("lobbyGatewayConnector update err : "..err.."\n"..traceback())
	end
end
