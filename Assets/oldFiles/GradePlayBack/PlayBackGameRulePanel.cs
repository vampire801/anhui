...
namespace PlayBack_1
{
    [Hotfix]
    [LuaCallCSharp]
    public class PlayBackGameRulePanel : MonoBehaviour
    {
        ...
        void SpwanPayNum(int MethodId)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                if (pbd.playingMethodConf.byBillingNumber == sum[i] && pbd.playingMethodConf.byBillingPrice == pay[i])
                {
                    ChoiceIndex = i;
                }
            }
            ...
            for (int i = 0; i < sum.Length; i++)
            {
                ...
                crcm.type = pbd.playingMethodConf.byBillingMode;
                crcm.num = sum[i];
                crcm.pay = pay[i];
                crcm.UpdateShow();
                count++;
        ...
        void SpwanMethod(int index)
        {
            ...
            for (int i = 0; i < mcm._dicMethodCardType[index].Count; i++)
            {
                ...
                bool status = JudgeIsShow(mcm._dicmethodToCardType[index][i].RuleId);
