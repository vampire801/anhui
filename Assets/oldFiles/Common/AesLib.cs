...
namespace Common
{
    ...
    public class AESCrypt
    {
        ...
        /// 密钥
        /// </summary>
        public const string KEY = "f83od0gmnr4y";
        ...
        /// 加密后的字符串
        /// </return>
        public static byte[] Encrypt(byte[] data, string strKey)
        {
            byte[] r = null;
            ...
            if (null != data)
            {
                try
                {
                    aes_ctx ctx = EncryptKey(strKey);

                    if (null != ctx)
                    {
                        r = EncryptBuf(data, ctx);
                    }
                }
                catch
                {
        ...
        /// 解密后的字符串
        /// </return>
        public static byte[] Decrypt(byte[] data, string strKey)
        {
            byte[] r = null;
            ...
            if (null != data)
            {
                try
                {
                    aes_ctx ctx = DecryptKey(strKey);

                    if (null != ctx)
                    {
                        r = DecryptBuf(data, ctx);
                    }
                }
                catch
                {
        ...
        /////////////////////////////////// decrypt ////////////////////////////////////////
        static aes_ctx DecryptKey(string strKey)
        {
            aes_ctx ctx = null;
            ...
            if (null != strKey)
            {
                ...
                byte[] sKey = Encoding.UTF8.GetBytes(strKey);

                if (FIX_KEY_LEN > sKey.Length)
                {
                    Array.Copy(sKey, Key, sKey.Length);
                    memset(Key, sKey.Length, 0x12);
                }
                else
                {
                    Array.Copy(sKey, Key, Key.Length);
                }

                uint[] ss = new uint[8];
                ctx = new aes_ctx();

                ctx.n_blk = (BLOCK_SIZE & ~3) | 2;

                int i;

                for (i = 0; i < 4; i++)
                {
                    ctx.k_sch[i] = ss[i] = BitConverter.ToUInt32(Key, i * 4);
                }

                kdf4(ctx.k_sch, 0, ss, fl_tab);

                for (i = 1; i < 9; i++)
                {
                    kd4(ctx.k_sch, i, ss, fl_tab);
                }

                kdl4(ctx.k_sch, i, ss, fl_tab);
        ...
        static byte[] DecryptBuf(byte[] data, aes_ctx ctx)
        {
            byte[] buf1 = new byte[BLOCK_LEN];
            byte[] buf2 = new byte[BLOCK_LEN];
            byte[] dbuf = new byte[2 * BLOCK_LEN];
            byte[] temp = new byte[BLOCK_LEN];
            int pos_in, pos_out, i, l, rlen = data.Length;
            byte[] r = new byte[rlen - BLOCK_LEN];
            ...
            if (2 * BLOCK_LEN > rlen)
            {
                Array.Copy(data, dbuf, rlen);
                l = rlen;
                l -= BLOCK_LEN;

                Array.Copy(dbuf, l, buf1, 0, BLOCK_LEN);
                DecryptBlk(buf1, buf1, ctx);
                Array.Copy(buf1, 0, dbuf, l, BLOCK_LEN);
                DecryptBlk(dbuf, dbuf, ctx);
            ...
            else
            {
                ...
                Array.Copy(data, buf1, BLOCK_LEN);
                pos_in += BLOCK_LEN;
                ...
                while (0 < rlen)
                {
                    if (BLOCK_LEN > rlen)
                    {
                        Array.Copy(data, pos_in, buf2, 0, rlen);
                        l = rlen;
                    }
                    else
                    {
                        Array.Copy(data, pos_in, buf2, 0, BLOCK_LEN);
                        pos_in += BLOCK_LEN;
                        l = BLOCK_LEN;
                    }
                    rlen -= l;
                    ...
                    if (BLOCK_LEN > rlen)
                    {
                        ...
                        if (0 < rlen)
                        {
                            Array.Copy(data, pos_in, dbuf, BLOCK_LEN, rlen);
                        }

                        Array.Copy(dbuf, rlen, temp, 0, BLOCK_LEN);
                        DecryptBlk(temp, temp, ctx);
                        Array.Copy(temp, 0, dbuf, rlen, BLOCK_LEN);
                        ...
                        Array.Copy(dbuf, buf2, BLOCK_LEN);
                        l = rlen + BLOCK_LEN;
                        rlen = 0;
                    }

                    DecryptBlk(buf2, dbuf, ctx);
        ...
        static void DecryptBlk(byte[] in_blk, byte[] out_blk, aes_ctx ctx)
        {
            uint[] b0 = new uint[4];
            uint[] b1 = new uint[4];
            int nc = BLOCK_SIZE >> 2;
            int i, kp = (int)(nc * ctx.n_rnd);

            //if(!(cx.n_blk & 2)) return;

            state_in(b0, in_blk, ctx.k_sch, kp);

            kp = 9 * nc;

            for (i = 0; i < 10; i++)
            {
                round((1 == i % 2) ? b0 : b1, (1 == i % 2) ? b1 : b0, ctx.k_sch, kp - i * nc, nc, (9 == i) ? il_tab : it_tab, false);
            }

            for (i = 0; i < 4; i++)
            {
                Array.Copy(BitConverter.GetBytes(b0[i]), 0, out_blk, i * 4, 4);
            }
        }

        static void inv_rnd(uint[] y, uint[] x, uint[] k, int index, int c, int nc, uint[][] tab)
        {
            y[c] = k[c + index] ^
                four_tables(tab, 0, inv_var(x, 0, c, nc), 0) ^
                four_tables(tab, 1, inv_var(x, 1, c, nc), 1) ^
                four_tables(tab, 2, inv_var(x, 2, c, nc), 2) ^
                four_tables(tab, 3, inv_var(x, 3, c, nc), 3);
        }

        static uint inv_var(uint[] x, int r, int c, int nc)
        {
            return
                r == 0 ?
                    (c == 0 ? x[0]
              ...
              : r == 1 ?
                    ...
                    : x[6])
              : r == 2 ?
                    (c == 0 ? nc == 4 ? x[2] : nc == 8 ? x[5] : x[4]
                    : c == 1 ? nc == 4 ? x[3] : nc == 8 ? x[6] : x[5]
                    : c == 2 ? nc == 8 ? x[7] : x[0]
                    : c == 3 ? nc == 8 ? x[0] : x[1]
                    : c == 4 ? nc == 8 ? x[1] : x[2]
                    : c == 5 ? nc == 8 ? x[2] : x[3]
                    ...
                    : x[4])
              :
                    (c == 0 ? nc == 4 ? x[1] : nc == 8 ? x[4] : x[3]
                    : c == 1 ? nc == 4 ? x[2] : nc == 8 ? x[5] : x[4]
                    : c == 2 ? nc == 4 ? x[3] : nc == 8 ? x[6] : x[5]
                    : c == 3 ? nc == 8 ? x[7] : x[0]
                    : c == 4 ? nc == 8 ? x[0] : x[1]
                    : c == 5 ? nc == 8 ? x[1] : x[2]
                    : c == 6 ? x[2]
        ...
        static void kdf4(uint[] k, int i, uint[] ss, uint[][] tab)
        {
            ...
            ss[3] = ss[3];
            ss[4] =
                four_tables(tab, 0, ss[(i + 3) % 4], rf2(0, 3)) ^
                four_tables(tab, 1, ss[(i + 3) % 4], rf2(1, 3)) ^
                four_tables(tab, 2, ss[(i + 3) % 4], rf2(2, 3)) ^
                four_tables(tab, 3, ss[(i + 3) % 4], rf2(3, 3)) ^ rcon_tab[i];
            ss[i % 4] ^= ss[4];
            ss[4] ^= k[4 * (i)];
            k[4 * (i) + 4] = ff(ss[4]);
            ss[4] ^= k[4 * (i) + 1];
        ...
        static void kd4(uint[] k, int i, uint[] ss, uint[][] tab)
        {
            ss[4] =
                four_tables(tab, 0, ss[(i + 3) % 4], rf2(0, 3)) ^
                four_tables(tab, 1, ss[(i + 3) % 4], rf2(1, 3)) ^
        ...
        static void kdl4(uint[] k, int i, uint[] ss, uint[][] tab)
        {
            ss[4] =
                four_tables(tab, 0, ss[(i + 3) % 4], rf2(0, 3)) ^
                four_tables(tab, 1, ss[(i + 3) % 4], rf2(1, 3)) ^
        ...
        static uint upr(uint x, uint n)
        {
            return (x << (int)(8 * n)) | (x >> (int)(32 - 8 * n));
        }

        static uint FFmulX(uint x)
        {
            return ((((x) & m2) << 1) ^ ((((x) & m1) >> 7) * BPOLY));
        }
        ...
        static uint inv_mcol(uint x)
        {
            ...
            f8 = FFmulX(f4);
            f9 ^= f8;
            return f2 ^= f4 ^ f8 ^ upr(f2 ^ f9, 3) ^ upr(f4 ^ f9, 2) ^ upr(f9, 1);
        }

        static uint ff(uint x)
        {
            return inv_mcol(x);
        }
        ...
        #endregion 解密用的私有成员方法

        #region 加密用的私有成员方法

        /////////////////////////////////// encrypt ////////////////////////////////////////
        static byte[] EncryptBuf(byte[] data, aes_ctx ctx)
        {
            byte[] buf = new byte[BLOCK_LEN];
            byte[] dbuf = new byte[2 * BLOCK_LEN];
            int i, l, rlen = data.Length;
            byte[] r = new byte[rlen + BLOCK_LEN];

            fillrand(dbuf, BLOCK_LEN); // set a random IV

            if (BLOCK_LEN > rlen)
            { // file length of less than one block
                Array.Copy(data, 0, dbuf, BLOCK_LEN, rlen);
                l = rlen;
                for (i = 0; i < l; ++i)
                { // xor file bytes with IV bytes
                    dbuf[i + BLOCK_LEN] ^= dbuf[i];
                }

                EncryptBlk(dbuf, dbuf, ctx);
                Array.Copy(dbuf, l, buf, 0, BLOCK_LEN);
                EncryptBlk(buf, buf, ctx);
                Array.Copy(dbuf, r, l);
                Array.Copy(buf, 0, r, l, BLOCK_LEN);
            }
            else
            {
                int pos_out = 0, pos_in = 0;
                EncryptBlk(dbuf, buf, ctx);
                Array.Copy(buf, r, BLOCK_LEN);
                pos_out = BLOCK_LEN;

                while (0 < rlen)
                {
                    Array.Copy(data, pos_in, buf, 0, BLOCK_LEN);
                    pos_in += BLOCK_LEN;
                    l = BLOCK_LEN;
                    rlen -= l;
                    ...
                    if (BLOCK_LEN > rlen)
                    {
                        ...
                        Array.Copy(dbuf, rlen, buf, 0, BLOCK_LEN);
                        EncryptBlk(buf, buf, ctx);
                        Array.Copy(buf, 0, dbuf, rlen, BLOCK_LEN);
                        l = rlen + BLOCK_LEN;
                        rlen = 0;
                    }
        ...
        static void EncryptBlk(byte[] in_blk, byte[] out_blk, aes_ctx ctx)
        {
            uint[] b0 = new uint[4];
            uint[] b1 = new uint[4];

            //if(!(cx.n_blk & 1)) return;

            state_in(b0, in_blk, ctx.k_sch, 0);

            int nc = BLOCK_SIZE >> 2;
            int i, kp = (int)(ctx.n_rnd - 9) * nc;

            for (i = 0; i < 10; i++)
            {
                round((1 == i % 2) ? b0 : b1, (1 == i % 2) ? b1 : b0, ctx.k_sch, kp + i * nc, nc, (9 == i) ? fl_tab : ft_tab, true);
            }
        ...
        static void fillrand(byte[] buf, int len)
        {
            byte[] t = { 0x60, 0x9A, 0x6C, 0xCE, 0x0D, 0x85, 0xA0, 0xB3, 0x76, 0x4C, 0xCC, 0x7D, 0x6B, 0x4D, 0x12, 0x8E };
            Array.Copy(t, buf, len);
        }

        private static void round(uint[] y, uint[] x, uint[] k, int index, int nc, uint[][] tab, bool bEncrypt)
        {
            for (int i = 0; i < 4; i++)
            {
                if (true == bEncrypt)
                {
                    fwd_rnd(y, x, k, index, i, nc, tab);
                }
                else
                {
                    inv_rnd(y, x, k, index, i, nc, tab);
                }
            }
        }

        static uint fwd_var(uint[] x, int r, int c, int nc)
        {
            return
               r == 0 ?
                (c == 0 ? x[0]
             ...
             : r == 2 ?
                (c == 0 ? nc == 8 ? x[3] : x[2]
                : c == 1 ? nc == 8 ? x[4] : x[3]
                : c == 2 ? nc == 4 ? x[0] : nc == 8 ? x[5] : x[4]
                : c == 3 ? nc == 4 ? x[1] : nc == 8 ? x[6] : x[5]
                : c == 4 ? nc == 8 ? x[7] : x[0]
                : c == 5 ? nc == 8 ? x[0] : x[1]
                : c == 6 ? x[1]
                : x[2])
                :
                (c == 0 ? nc == 8 ? x[4] : x[3]
                : c == 1 ? nc == 4 ? x[0] : nc == 8 ? x[5] : x[4]
                : c == 2 ? nc == 4 ? x[1] : nc == 8 ? x[6] : x[5]
                : c == 3 ? nc == 4 ? x[2] : nc == 8 ? x[7] : x[0]
                : c == 4 ? nc == 8 ? x[0] : x[1]
                : c == 5 ? nc == 8 ? x[1] : x[2]
                : c == 6 ? x[2]
                : x[3]);
        ...
        static void fwd_rnd(uint[] y, uint[] x, uint[] k, int index, int c, int nc, uint[][] tab)
        {
            y[c] = k[c + index] ^
                four_tables(tab, 0, fwd_var(x, 0, c, nc), 0) ^
                four_tables(tab, 1, fwd_var(x, 1, c, nc), 1) ^
                four_tables(tab, 2, fwd_var(x, 2, c, nc), 2) ^
                four_tables(tab, 3, fwd_var(x, 3, c, nc), 3);
        }

        static void state_in(uint[] y, byte[] x, uint[] k, int index)
        {
            for (int i = 0; i < 4; i++)
            {
                y[i] = BitConverter.ToUInt32(x, i * 4) ^ k[i + index];
            }
        }

        static aes_ctx EncryptKey(string strKey)
        {
            aes_ctx ctx = null;
        ...
        static void ke4(uint[] k, int i, uint[] ss)
        {
            k[4 * i + 4] = ss[0] ^=
                four_tables(fl_tab, 0, ss[3], rf2(0, 3)) ^
                four_tables(fl_tab, 1, ss[3], rf2(1, 3)) ^
                four_tables(fl_tab, 2, ss[3], rf2(2, 3)) ^
                four_tables(fl_tab, 3, ss[3], rf2(3, 3)) ^ rcon_tab[i];
            k[4 * i + 5] = ss[1] ^= ss[0];
            k[4 * i + 6] = ss[2] ^= ss[1];
            k[4 * i + 7] = ss[3] ^= ss[2];
        }

        static uint four_tables(uint[][] tab, uint m, uint vf, uint rf)
        {
            return tab[m][bval(vf, rf)];
        }

        static byte bval(uint x, uint n)
        {
            return (byte)(x >> (int)(8 * n));
        }

        static uint rf2(uint r, uint c)
        {
            return (r - c) & 3;
        }
        /////////////////////////////////// encrypt ////////////////////////////////////////

        #endregion 加密用的私有成员方法

        #region 其他私有成员

        static void memset(byte[] buff, int index, byte val)
        {
        ...
        class aes_ctx
        {
            public uint[] k_sch = new uint[KS_LENGTH];
            public uint n_rnd;
            public uint n_blk;
        }

        const int BPOLY = 0x1b;
        const uint m1 = 0x80808080;
        const int m2 = 0x7f7f7f7f;
        const int BLOCK_LEN = 16;
        const int BLOCK_SIZE = 16;
        const int KS_LENGTH = 4 * BLOCK_SIZE;
        const int FIX_KEY_LEN = 16;

        static uint[] rcon_tab = new uint[29] {
			...
			0x000000b3, 0x0000007d, 0x000000fa, 0x000000ef, 0x000000c5
		};

        static uint[][] fl_tab = new uint[4][]{
			new uint[256] {
