...
namespace AssetBundles
{
	internal class LaunchAssetBundleServer : ScriptableSingleton<LaunchAssetBundleServer>
	{
		...
		{
			KillRunningAssetBundleServer();
			WriteAssetBundleServerURL(ChannelType.Test.ToString());
        ...
        public static void WriteAssetBundleServerURL(string channelName)
        {
            var path = GetStreamingAssetBundleServerUrl();
            BaseChannel channel = ChannelManager.instance.CreateChannel(channelName);
            GameUtility.SafeWriteAllText(path, channel.GetAssetBundleServerUrl());
